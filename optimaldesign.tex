...
\Fref{fig:optimaluniform} shows the result for the optimal design region for uniformly distributed $x$ input parameters (as described in \sref{seq:exopt}) in the case of this 4-parameter model, giving $\tilde x = 2.808$ for the model parameters listed. If the used input parameters come form just the middle region region (small $\tilde x$) the offset cannot be estimated very well, while with large region too small fraction of the measurements will fall into central area where one could observe the the Lorentzian response itself.
